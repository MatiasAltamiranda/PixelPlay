...
const GameState = (prop) => {
  ...
  const getGamesCategory = async (category)=>{
    try {
     const response = await clientAxios.get(`/api/v1/games/category/${category}`)
...
const updateGame = async (id,data)=>{
  ...
  return (
    <GamesContext.Provider value={{ ...state, createGame,getGamesCategory,getGame,getGames,deleteGame,getOneGame,updateGame}}>
      {children}
